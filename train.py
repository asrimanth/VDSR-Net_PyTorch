...
class Trainer:
    def __init__(self, train_dataloader, valid_dataloader, config):
        ...
        Args:
            ...
            config (Object): An object of the class Configuration, which can be modified in config.py
        """
        self.config = config
        self.patience = self.config.PATIENCE_EARLY_STOPPING
        self.model = VDSR_Net_v2(depth=self.config.DEPTH)
        ...
        self.log = pd.DataFrame(columns=["model_name", "train_loss", "train_PSNR", 
    ...
    def validation_step(self):
        ...
        ssim_valid = 0
        with torch.no_grad():
            for lr_image, hr_image, res_diff in tqdm(self.valid_dataloader, total=len(self.valid_dataloader)):
                low_res_image = lr_image.to(self.device)
                high_res_image = hr_image.to(self.device)
                res_diff = res_diff.to(self.device)
                output = self.model(low_res_image, res_diff)
                loss = self.loss_function(output.data, high_res_image)
                loss_valid += loss.item()
                psnr_valid += self.PSNR(output.data.to("cpu"), high_res_image.cpu())
                ssim_valid += self.SSIM(output.data.to("cpu"), high_res_image.cpu())
    ...
    def fit(self):
        run = wandb.init(project="VDSR-DIV2K", entity="asrimanth")
        run.save()
        ...
        for epoch in range(self.epochs):
            ...
            for lr_image, hr_image, res_diff in tqdm(self.train_dataloader, total=len(self.train_dataloader)):
                ...
                psnr_train += self.PSNR(output.data.to("cpu"), hr_image.to("cpu"))
                ssim_train += self.SSIM(output.data.to("cpu"), hr_image.to("cpu"))
                b_num += 1
...
if __name__ == "__main__":
    ...
    train_dataset = StandardDataset(config.TRAIN_PATH, image_transforms=train_transforms)
    valid_dataset = StandardDataset(config.VALID_PATH, image_transforms=valid_transforms)
