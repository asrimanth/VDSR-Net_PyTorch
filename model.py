...
class VDSR_Net_v2(nn.Module):
    ...
    def forward(self, lr_image, residual):
        residual = self.input_block(residual)
        residual = self.vdsr_blocks(residual)
        residual = self.output_block(residual)
        residual = torch.add(lr_image, residual)
        return residual
