...
def test_report(model, test_dataset, config):
    ...
    for low_res_image, gt_tensor, residual in tqdm(test_dataset):
        ...
        output = model(low_res_image.unsqueeze(0).to(config.DEVICE), residual.unsqueeze(0).to(config.DEVICE))
        ...
        images_list = [low_res_image, output, gt_tensor, get_difference(low_res_image, output), get_difference(low_res_image, gt_tensor)]
...
if __name__ == "__main__":
    ...
    model = VDSR_Net_v2()
    model.load_state_dict(torch.load("./models/VDSR-Net_32_v5.pth"))
    model.eval()
